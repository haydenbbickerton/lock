...
class PrivilegeSpec extends ObjectBehavior
{
    function let()
    {
        $this->beConstructedWith('edit', new SimpleTarget('events', 1));
    }
    ...
    {
        $this->isAllowed($lock, 'edit', new SimpleTarget('events', 1))->shouldReturn(true);
        $this->isAllowed($lock, 'edit', new SimpleTarget('events', 1))->shouldReturn(true);
        $this->isAllowed($lock, 'edit', new SimpleTarget('events', 2))->shouldReturn(false);
        $this->isAllowed($lock, 'delete', new SimpleTarget('comments', 1))->shouldReturn(false);
    }
    ...
    {
        $this->beConstructedWith('edit', new SimpleTarget('events', 1), function () {
            return false;
        });

        $this->isAllowed($lock, 'edit', new SimpleTarget('events', 1))->shouldReturn(false);
    }
}
